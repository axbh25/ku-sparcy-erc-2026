"""Launch-construction helper for the sequential Day 4 -> Day 8 pipeline. Original Day 7 helper is frozen."""
...
VALID_STOPS = ('day5', 'day6', 'day7', 'day8')
...
def _after_process(event, context, *, stage: str, next_action, stop_after):
    ...
    if next_action is None:
        return [
            LogInfo(msg='[PIPELINE][PASS] Day 8 completed'),
            EmitEvent(event=Shutdown(reason='Day 8 completed')),
        ]
    return [
...
def generate_pipeline_description(default_stop_after: str) -> LaunchDescription:
    ...
    day8_path = LaunchConfiguration('day8_result_path')
    ...
    day8 = Node(
        package='ku_sparcy_erc', executable='day8_bin_place',
        name='ku_sparcy_day8_bin_place', output='screen', emulate_tty=True,
        parameters=[config('day8_place.yaml'), {
            'use_sim_time': True,
            'operation': 'execute',
            'manual_approval_required': False,
            'day7_result_path': ParameterValue(day7_path, value_type=str),
            'result_path': ParameterValue(day8_path, value_type=str),
            'image_output_dir': ParameterValue(image_dir, value_type=str),
        }],
    )
    ...
    handlers = [
        ...
        RegisterEventHandler(OnProcessExit(
            ...
            on_exit=lambda event, context: _after_process(
                ...
                next_action=day8, stop_after=stop_after),
        )),
        RegisterEventHandler(OnProcessExit(
            target_action=day8,
            on_exit=lambda event, context: _after_process(
                event, context, stage='day8',
    ...
    actions: List[object] = [
        ...
        DeclareLaunchArgument(
            ...
            description='Development checkpoint. Competition solution uses day8.'),
        DeclareLaunchArgument(
            'image_output_dir',
        ...
        DeclareLaunchArgument(
            'day8_result_path',
            default_value='/opt/erc_ws/src/ku_sparcy_erc/day8_results/day8_result.json'),
        *handlers,
        home,
    ]
